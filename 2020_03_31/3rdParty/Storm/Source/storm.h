...
BOOL STORMAPI SNetCreateGame(const char *pszGameName, const char *pszGamePassword, const char *pszGameStatString, DWORD dwGameType, void *GameTemplateData, int GameTemplateSize, int playerCount, char *creatorName, char *a11, DWORD *playerID);
...
BOOL STORMAPI SNetReceiveMessage(DWORD *senderplayerid, char **data, DWORD *databytes);
BOOL STORMAPI SNetReceiveTurns(int a1, int arraysize, void *arraydata, void *arraydatabytes, void *arrayplayerstatus);
...
BOOL STORMAPI SFileDdaGetPos(HANDLE directsound, DWORD *a2, DWORD *a3);
...
BOOL STORMAPI SRegLoadValue(const char *keyname, const char *valuename, BYTE flags, DWORD *value);
...
BOOL STORMAPI SVidPlayBegin(const char *filename, int arg4, int a3, int a4, int a5, int a6, HANDLE* video);
...
//
// void STORMAPI SErrReportNamedResourceLeak(const char *pszMsg, const char *pszSubMsg)
...
_bool __cdecl StormDestroy(void);
_bool __stdcall SFileSetBasePath(char *);
void __cdecl SDrawRealizePalette(void);
BOOL __stdcall SVidPlayContinue(void);
...
_bool __stdcall SNetSetBasePlayer(int);
